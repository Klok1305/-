...
Эквивалентно: $\mathcal{T}(v_{\sigma(1)}, \ldots, v_{\sigma(q)}) = \mathcal{T}(v_1,\ldots,v_q)$ (симм.) или $= \operatorname{sgn}(\sigma) \cdot \mathcal{T}(v_1,\ldots,v_q)$ (косс.) для любой перестановки $\sigma \in S_q$.
...
- Кососимметрический: $T_{j_1\ldots j_q} = \operatorname{sgn}(\sigma)\, T_{j_{\sigma(1)}\ldots j_{\sigma(q)}}$ для всех $\sigma \in S_q$.
...
Симметрический $\mathcal{T}$: $\mathcal{T}(v_{\sigma(1)},\ldots) = \mathcal{T}(v_1,\ldots)$. Кососимметрический: $\mathcal{T}(v_{\sigma(1)},\ldots) = \operatorname{sgn}(\sigma)\mathcal{T}(v_1,\ldots)$. Следствие: если два аргумента равны, то $\mathcal{T} = 0$ (для косс.). В координатах: $T_{j_{\sigma(1)}\ldots} = \operatorname{sgn}(\sigma) T_{j_1\ldots}$, и $T_{\ldots k \ldots k \ldots} = 0$.
...
**Операция симметризации** $\operatorname{Sym}: T^0_q(V) \to S^q(V)$:

$$(\operatorname{Sym}\, T)_{j_1\ldots j_q} = \frac{1}{q!} \sum_{\sigma \in S_q} T_{j_{\sigma(1)}\ldots j_{\sigma(q)}}.$$

Геометрически: $(\operatorname{Sym}\, \mathcal{T})(v_1,\ldots,v_q) = \frac{1}{q!} \sum_{\sigma \in S_q} \mathcal{T}(v_{\sigma(1)},\ldots,v_{\sigma(q)})$.

**Операция альтернирования (антисимметризации)** $\operatorname{Alt}: T^0_q(V) \to \Lambda^q(V^*)$:

$$(\operatorname{Alt}\, T)_{j_1\ldots j_q} = \frac{1}{q!} \sum_{\sigma \in S_q} \operatorname{sgn}(\sigma)\, T_{j_{\sigma(1)}\ldots j_{\sigma(q)}}.$$

Геометрически: $(\operatorname{Alt}\, \mathcal{T})(v_1,\ldots,v_q) = \frac{1}{q!} \sum_{\sigma \in S_q} \operatorname{sgn}(\sigma)\, \mathcal{T}(v_{\sigma(1)},\ldots,v_{\sigma(q)})$.

**Свойство быть проектором.** Оба оператора — проекторы на соответствующие подпространства:

$$\operatorname{Sym}^2 = \operatorname{Sym}, \quad \operatorname{Alt}^2 = \operatorname{Alt}.$$

Более того:
- Если $T$ уже симметрический, то $\operatorname{Sym}\, T = T$.
- Если $T$ уже кососимметрический, то $\operatorname{Alt}\, T = T$.

### 3. Уровень 2 — для четвёрки

**Доказательство $\operatorname{Alt}^2 = \operatorname{Alt}$:** Пусть $T' = \operatorname{Alt}\, T$. Нужно показать $\operatorname{Alt}\, T' = T'$.

$$(\operatorname{Alt}\, T')_{j_1\ldots j_q} = \frac{1}{q!} \sum_\tau \operatorname{sgn}(\tau)\, T'_{j_{\tau(1)}\ldots j_{\tau(q)}}.$$

Но $T'_{j_{\tau(1)}\ldots j_{\tau(q)}} = \operatorname{sgn}(\tau) T'_{j_1\ldots j_q}$ (т.к. $T'$ кососимметрический). Значит:

$$(\operatorname{Alt}\, T')_{j_1\ldots j_q} = \frac{1}{q!} \sum_\tau \operatorname{sgn}^2(\tau)\, T'_{j_1\ldots j_q} = \frac{1}{q!} \cdot q! \cdot T'_{j_1\ldots j_q} = T'_{j_1\ldots j_q}.$$

**Примеры:**

$\operatorname{Sym}\,$ для $q=2$: $(\operatorname{Sym}\, T)_{ij} = \frac{T_{ij}+T_{ji}}{2}$.

$\operatorname{Alt}\,$ для $q=2$: $(\operatorname{Alt}\, T)_{ij} = \frac{T_{ij}-T_{ji}}{2}$.

Любая билинейная форма разлагается: $T = \operatorname{Sym}\, T + \operatorname{Alt}\, T$ (сумма симметрической и кососимметрической).

### 4. Мини-конспект

Симметризация: $(\operatorname{Sym}\, T)_{j_1\ldots j_q} = \frac{1}{q!}\sum_\sigma T_{j_{\sigma(1)}\ldots j_{\sigma(q)}}$.

Альтернирование: $(\operatorname{Alt}\, T)_{j_1\ldots j_q} = \frac{1}{q!}\sum_\sigma \operatorname{sgn}(\sigma) T_{j_{\sigma(1)}\ldots j_{\sigma(q)}}$.
...
$$P \wedge Q = \frac{(p+q)!}{p!\, q!}\, \operatorname{Alt}(P \otimes Q).$$

В координатах:

$$(P \wedge Q)_{j_1\ldots j_{p+q}} = \frac{(p+q)!}{p!\,q!} \cdot \frac{1}{(p+q)!} \sum_{\sigma \in S_{p+q}} \operatorname{sgn}(\sigma)\, P_{j_{\sigma(1)}\ldots j_{\sigma(p)}} Q_{j_{\sigma(p+1)}\ldots j_{\sigma(p+q)}}.$$

Это эквивалентно:

$$(P \wedge Q)_{j_1\ldots j_{p+q}} = \frac{1}{p!\,q!} \sum_{\sigma \in S_{p+q}} \operatorname{sgn}(\sigma)\, P_{j_{\sigma(1)}\ldots j_{\sigma(p)}} Q_{j_{\sigma(p+1)}\ldots j_{\sigma(p+q)}}.$$
...
**Нормировка Ершова vs Панова.** В учебнике Ершова $P \wedge Q := \operatorname{Alt}(P \otimes Q)$ (без коэффициента $(p+q)!/(p!q!)$). В Панове — с этим коэффициентом. На экзамене использовать соглашение Панова.

**Производная Ли, дифференциальные формы** — приложения внешней алгебры в дифференциальной геометрии.

### 5. Доказательства

**Антикоммутативность (идея):** $P \wedge Q = \frac{(p+q)!}{p!q!}\operatorname{Alt}(P\otimes Q)$, аналогично $Q \wedge P = \frac{(p+q)!}{p!q!}\operatorname{Alt}(Q\otimes P)$. Нужно показать $\operatorname{Alt}(P\otimes Q) = (-1)^{pq}\operatorname{Alt}(Q\otimes P)$.
...
**Определитель через внешнее произведение:** 
$v_1 \wedge \cdots \wedge v_n = (v_1^{j_1} e_{j_1})\wedge\cdots = v_1^{j_1}\cdots v_n^{j_n}(e_{j_1}\wedge\cdots\wedge e_{j_n})$. Единственный ненулевой член при всех различных $j_1,\ldots,j_n$ даёт $\sum_\sigma \operatorname{sgn}(\sigma)\prod v_i^{\sigma(i)} \cdot e_1\wedge\cdots\wedge e_n = \det(v_1,\ldots,v_n) \cdot e_1\wedge\cdots\wedge e_n$.
...
- Путать нормировки: по Панову $P \wedge Q = \frac{(p+q)!}{p!q!}\operatorname{Alt}(P\otimes Q)$; по Ершову — без коэффициента.
...
| 52 | Симм-ция, альт-ние как проекторы | Формулы $\operatorname{Sym}$, $\operatorname{Alt}$; $\operatorname{Alt}^2=\operatorname{Alt}$ | $\operatorname{Alt}^2 = \operatorname{Alt}$ | ★★★ | ★★ |
| 53 | $\wedge$, внешняя алгебра, базис | $P\wedge Q = \frac{(p+q)!}{p!q!}\operatorname{Alt}(P\otimes Q)$; базис; $\det$ через $\wedge$ | Антикоммут., базис | ★★★ | ★★★ |

### Если времени мало

1. Кос.: $T(\ldots,v_i,\ldots,v_j,\ldots) = -T(\ldots,v_j,\ldots,v_i,\ldots)$; при $v_i=v_j$: $= 0$.
2. $\operatorname{Alt}(T)_{j_1\ldots} = \frac{1}{q!}\sum_\sigma \operatorname{sgn}(\sigma)T_{j_{\sigma(1)}\ldots}$; это проектор.
3. $P\wedge Q = \frac{(p+q)!}{p!q!}\operatorname{Alt}(P\otimes Q)$.
...
3. Запишите формулу $\operatorname{Alt}$ для $q=2$.
4. Почему $\operatorname{Alt}^2 = \operatorname{Alt}$?
...
1. Найдите $\operatorname{Alt}(T)$ для $T_{ij}$ — произвольной матрицы $2\times2$.
...
**Сразу:** запомнить формулы $\operatorname{Alt}$ и $\wedge$, базис $\Lambda^q$.  
**Вечером:** доказать $\operatorname{Alt}^2 = \operatorname{Alt}$ и антикоммутативность.  
